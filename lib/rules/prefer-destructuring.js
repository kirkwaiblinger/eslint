    ...
    create(context) {
        ...
        function shouldFix(node) {
            ...
            if (node.type === "AssignmentExpression") {
                assignmentRhs = node.right;
                assignmentLhs = node.left;
            } else if (node.type === "VariableDeclarator") {
                assignmentRhs = node.init;
                assignmentLhs = node.id;
            } else {
                return false;
            }

            return assignmentLhs.type === "Identifier" &&
                assignmentRhs.type === "MemberExpression" &&
                !assignmentRhs.computed && assignmentRhs.property.type === "Identifier" &&
                assignmentLhs.name === assignmentRhs.property.name;
        }
        ...
        function fixIntoObjectDestructuring(fixer, node) {
            const rightNode = node.type === "VariableDeclarator" ? node.init : node.right;
            ...
            let destructuringText = `{${rightNode.property.name}} = ${objectText}`;

            if (node.type === "AssignmentExpression") {
                destructuringText = `(${destructuringText})`;
            }
